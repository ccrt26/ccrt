...
| 7 | G4/G5 验收与补修 | 通常否 | 只有 WARN 是否接受、非关键能力是否延期、是否允许进入 G6 放行归档需要用户确认 | 执行自检、复查候选、补修、证据汇总；不得直接进入第 8 步 |
| 7.5 | G6 放行归档 | 是 | 是否确认基于 G5 通过结论进入 G6；是否确认归档、同步和阶段闭环 | 执行 G6 放行归档、归档证据、阶段完成记录；完成后才允许开启第 8 步 |
| 8 | Phase 2/3 产品化扩展 | 是 | 是否确认 Phase 1 已完成 G6 归档；今日驾驶舱第一屏看什么；股票详情页怎么组织；规则健康页看哪些指标；哪些操作一键化 | 设计后端增强、前端/驾驶舱、长期治理路线 |
...
7. 进入前端/驾驶舱设计时。
8. 进入 G6 放行/归档前。
9. 第 7 步 G4/G5 完成后，必须先完成第 7.5 步 G6 放行归档，不得直接进入第 8 步。
