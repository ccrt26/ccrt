...
function New-OoxmlDoc {
    ...
    AddHeading($bodySb, "3.4.1 均线系统(8分)", 3)
    AddTable($bodySb, @("条件","得分","判定"), @(
        ,@("MA5>MA10>MA20>MA60多头排列","8","强势多头"),
        ,@("MA5>MA10>MA20短中期多头","6","偏强"),
        ,@("MA5>MA10短期多头","4","中性偏强"),
        ,@("MA10>MA5短期死叉","2","弱势"),
        ,@("MA10≤MA20中期空头","0且否决","趋势走坏")
    ))

    AddHeading($bodySb, "3.4.2 RSI(5分)", 3)
    ...
    AddHeading($bodySb, "3.4.3 MACD(5分)", 3)
    ...
    AddHeading($bodySb, "3.4.4 量价配合(4分)", 3)
    AddTable($bodySb, @("条件","得分"), @(
        ,@("放量上涨(涨幅>2%且量比>1.5)","4"),
        ,@("缩量上涨(涨幅>0且量比<1.2)","3"),
        ,@("正常量价","2"),
    ...
    AddHeading($bodySb, "3.4.5 布林带(3分)", 3)
    AddTable($bodySb, @("条件","得分"), @(
        ,@("价格在中轨至上轨间，中轨向上","3"),
        ,@("价格在中轨附近，走平","2"),
        ,@("价格在中轨至下轨间","1"),
...
# Main generation
$docPath = [System.IO.Path]::GetFullPath("C:\Users\34269\Documents\Claude\股票分析\每日荐股\分析逻辑\每日荐股分析逻辑白皮书_v2.0.docx")
