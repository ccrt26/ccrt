...
$TASK_MAP += @{
    ...
    Task     = '评分/选股'
    WP       = '每日荐股分析逻辑白皮书 v2.4'
    WPath    = '每日荐股\分析逻辑\每日荐股分析逻辑白皮书_v2.4.md'
    Reqs     = @(
        '一票否决条件: -7%硬止损/近5日涨幅<5%/收盘<MA20*0.95',
        '评分维度权重: 时间25%+盈利30%+技术25%+资金20%',
...
$TASK_MAP += @{
    ...
    Task     = '每日荐股后评估'
    WP       = '次日后评估白皮书 v1.5'
    WPath    = '每日荐股\事后评估\次日后评估白皮书_v1.5.md'
    Reqs     = @(
...
$TASK_MAP += @{
    ...
    Task     = '重点股票深度分析'
    WP       = '重点股票跟踪分析逻辑白皮书 v2.0'
    WPath    = '重点股票\分析逻辑\重点股票跟踪分析逻辑白皮书_v2.0.md'
    Reqs     = @(
        '多周期融合: 短期/中期/长期三维预测',
        '证据加权: 多个独立维度同向方可形成结论',
