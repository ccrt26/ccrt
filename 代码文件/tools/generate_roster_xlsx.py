...
members = [
    ...
    ["宏观巡检官", "Sentinel", "/Sentinel", "政策信号、经济日历、市场情绪 → 支撑腰子宏观判断", "告诉腰子外部环境怎么变",
     ".claude/agents/宏观巡检-Sentinel.md", ".claude/commands/Sentinel.md"],
    ["数据监理", "Pulse", "/Pulse", "API巡检、缓存审计、数据完整性 → 支撑腰子数据信心", "告诉腰子数据能不能用",
     ".claude/agents/数据监理-Pulse.md", ".claude/commands/Pulse.md"],
    ["风控官", "Vega", "/Vega", "风险审计、过拟合检测、交易纪律 → 支撑腰子风险约束", "告诉腰子风险边界在哪里",
     ".claude/agents/风控官-Vega.md", ".claude/commands/Vega.md"],
    ["策略研究员", "Alpha", "/Alpha", "因子有效性、策略优化、规律挖掘 → 支撑腰子策略进化", "告诉腰子怎么让策略更聪明",
...
detail_data = [
    ...
    ("腰子 — 金融业务负责人", "金融分析决策核心；善用团队：宏观→Sentinel、数据→Pulse、风险→Vega、策略→Alpha；六维评分+综合判断；对金融判断质量负责"),
    ("Sentinel — 宏观巡检官", "每日盘前宏观简报（政策+数据+事件+情绪）；经济日历前瞻；政策信号解读；为腰子提供宏观环境判断"),
    ("Pulse — 数据监理", "三大API连通性巡检；缓存新鲜度审计、数据完整性验证；1+2架构合规检查；为腰子保障数据质量"),
    ("Vega — 风控官", "持仓集中度、相关性、流动性、事件风险；策略过拟合检测、回撤监控、交易纪律审查；为腰子设定风险边界"),
    ("Alpha — 策略研究员", "因子IC/ICIR/分层收益/衰退趋势；策略优化提案；新因子探索、规律挖掘；为腰子提供策略优化提案"),
    ("Arch — 系统架构师", "系统模块设计与边界划分、模块间接口契约定义；技术选型评估；数据流设计；重构决策；不写代码，出设计文档和架构图"),
    ("Forge — 构建工程师(CI)", "CI/CD流水线设计；自动化调度编排；定时任务管理；代码生成方案；不写代码，出流水线设计和调度方案"),
...
ws3.cell(row=4, column=1, value="Sentinel+Pulse+Vega+Alpha → 腰子(综合判断) → 投资决策").font = Font(name="Microsoft YaHei", color="16213E", size=10, bold=True)
...
chain_data = [
    ("宏观环境", "Sentinel — 政策信号 / 经济日历 / 市场情绪 → 输出给腰子"),
    ("数据管线", "Pulse — API巡检 / 缓存审计 / 完整性验证 → 输出给腰子"),
    ("分析决策(核心)", "腰子 — 六维评分 / 综合判断 / 最终决策 ← 接收四方支撑"),
    ("风险审计", "Vega — 集中度 / 过拟合 / 纪律审查 → 输出给腰子"),
    ("策略进化", "Alpha — 因子IC / 参数优化 / 规律挖掘 → 输出给腰子"),
...
rules = [
    ...
    ("支撑角色向腰子汇报", "Sentinel/Pulse/Vega/Alpha的输出统一给腰子；腰子不在时，阿黑代为接收"),
    ("分工明确，互不越界", "腰子不做风控细节，Vega不挖因子，Alpha不看单票，Sentinel不做个股；Craft只写代码"),
    ("金融信息流", "Sentinel+Pulse+Vega+Alpha → 腰子 → 投资决策"),
    ("工程信息流", "Arch设计 → Forge构建 → Dock部署 → Proof验证 → Craft实现"),
...
files = [
    ...
    ("Vega角色定义", ".claude/agents/风控官-Vega.md", "风控官完整人设 (v2.1)"),
    ("Pulse角色定义", ".claude/agents/数据监理-Pulse.md", "数据监理完整人设 (v2.1)"),
    ("Alpha角色定义", ".claude/agents/策略研究员-Alpha.md", "策略研究员完整人设 (v2.1)"),
    ("Sentinel角色定义", ".claude/agents/宏观巡检-Sentinel.md", "宏观巡检完整人设 (v2.1)"),
    ("", "", ""),
    ("工程团队角色定义", "", ""),
    ("Arch角色定义", ".claude/agents/系统架构师-Arch.md", "系统架构师完整人设 (v2.0)"),
    ("Forge角色定义", ".claude/agents/构建工程师-Forge.md", "构建工程师(CI)完整人设 (v2.0)"),
    ("Dock角色定义", ".claude/agents/部署工程师-Dock.md", "部署工程师(环境)完整人设 (v2.0)"),
    ...
    ("Vega知识库 (6+1文件)", ".claude/agents/Vega-知识库/", "风控知识体系"),
    ("Alpha知识库 (5+1文件)", ".claude/agents/Alpha-知识库/", "策略研究知识体系"),
    ("Sentinel知识库 (6+1文件)", ".claude/agents/Sentinel-知识库/", "宏观巡检知识体系"),
    ("Pulse知识库 (5+1文件)", ".claude/agents/Pulse-知识库/", "数据监理知识体系"),
    ("Arch知识库 (5+1文件)", ".claude/agents/Arch-知识库/", "系统架构知识体系"),
    ("Forge知识库 (4+1文件)", ".claude/agents/Forge-知识库/", "构建工程知识体系"),
