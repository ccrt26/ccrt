...
def diff_numstat(base: str) -> Dict[str, Tuple[Optional[int], Optional[int]]]:
    result = run_git(["diff", "--numstat", f"{base}..HEAD"])
...
def blocked_or_warn(status: str, lane: str, message: str, files: Iterable[str] = ()) -> Finding:
...
def check_branch(branch: str, allow_main: bool) -> Finding:
    ...
    if branch in MAIN_BRANCHES and not allow_main:
        return blocked_or_warn("BLOCK", "阿黑-任务边界", f"当前分支是 {branch}，禁止直接上传主线。")
    if branch.startswith("codex/") or branch.startswith("codex-"):
        return blocked_or_warn("PASS", "阿黑-任务边界", f"任务分支合规：{branch}")
    return blocked_or_warn("WARN", "阿黑-任务边界", f"分支 {branch} 可用，但建议使用 codex/ 或 codex- 前缀。")
...
def check_working_tree(files: List[ChangedFile], allow_dirty: bool) -> Finding:
    ...
    if allow_dirty:
        return blocked_or_warn("WARN", "旧影-审计基线", f"工作区存在 {len(files)} 个未提交变更。", paths[:20])
    return blocked_or_warn("BLOCK", "旧影-审计基线", f"工作区存在 {len(files)} 个未提交变更，上传前必须清理。", paths[:20])
...
def check_backlog_threshold(files: List[ChangedFile]) -> Finding:
    ...
    if len(files) > MAX_DIRTY_FILES:
        return blocked_or_warn("BLOCK", "阿黑-积压阈值", f"工作区变更 {len(files)} 个，超过 {MAX_DIRTY_FILES}。", [f.path for f in files[:20]])
    if len(untracked) > MAX_UNTRACKED_FILES:
        return blocked_or_warn("BLOCK", "阿黑-积压阈值", f"未跟踪文件 {len(untracked)} 个，超过 {MAX_UNTRACKED_FILES}。", untracked[:20])
    if files:
        return blocked_or_warn("WARN", "阿黑-积压阈值", f"工作区有 {len(files)} 个变更、{len(untracked)} 个未跟踪。")
...
def check_runtime_files(files: List[ChangedFile]) -> Finding:
    ...
    if runtime:
        return blocked_or_warn("BLOCK", "旧影-垃圾文件", "运行日志或临时文件混入变更。", runtime[:30])
...
def check_data_size(files: List[ChangedFile]) -> Finding:
    ...
    if large:
        return blocked_or_warn("WARN", "玉夜-数据边界", "存在大体量 diff，需在 PR 中说明是否可复现。", large[:20])
    data_count = sum(1 for f in files if f.category == "data")
    return blocked_or_warn("PASS", "玉夜-数据边界", f"数据变更数量：{data_count}。")
...
def check_reports(files: List[ChangedFile], allow_production: bool) -> Finding:
    ...
    if reports and not allow_production:
        return blocked_or_warn("BLOCK", "腰子-发布边界", "正式报告或发布产物变更未显式放行。", reports[:30])
    if reports:
        return blocked_or_warn("WARN", "腰子-发布边界", "包含正式报告或发布产物，必须人工审阅。", reports[:30])
...
def check_code_scope(files: List[ChangedFile]) -> Finding:
    ...
    return blocked_or_warn("WARN", "情墨-代码影响面", f"代码变更 {len(code_files)} 个，需配套验证。", code_files[:30])


def check_validation(files: List[ChangedFile]) -> Finding:
    has_code = any(f.category == "code" for f in files)
    has_test = any("/test" in f.path or f.path.startswith("tests/") or "validate_" in f.path for f in files)
    if has_code and not has_test:
        return blocked_or_warn("WARN", "新安-质量验证", "包含代码变更但未发现测试/验证文件同步变更。")
    if has_code:
        return blocked_or_warn("PASS", "新安-质量验证", "代码变更包含测试或验证相关文件。")
    return blocked_or_warn("PASS", "新安-质量验证", "本次无需代码测试配套。")


def check_upstream(upstream: str) -> Finding:
    if not upstream:
        return blocked_or_warn("WARN", "阿黑-GitHub承接", "当前分支尚未设置 upstream，首次上传需 git push -u。")
...
def evaluate(args: argparse.Namespace) -> Dict[str, object]:
    ...
    findings = [
        ...
        check_working_tree(local_changes, args.allow_dirty),
        check_backlog_threshold(local_changes),
...
def render_report(report: Dict[str, object], as_json: bool) -> None:
    ...
    emit(f"branch={report['branch']} upstream={report['upstream'] or '-'} base={report['base']}")
    emit(f"working_tree={report['working_tree_count']} branch_changes={report['branch_change_count']}")
...
def command_classify(args: argparse.Namespace) -> int:
    files = branch_changes(args.base or DEFAULT_BASE) if args.compare_branch else working_tree_changes()
    rows = [{"path": f.path, "status": f.status, "category": f.category, "lines": f.total_lines} for f in files]
    emit(json.dumps(rows, ensure_ascii=False, indent=2) if args.json else "\n".join(f"{r['category']}\t{r['status']}\t{r['path']}" for r in rows))
    return 0
...
def command_snapshot(args: argparse.Namespace) -> int:
    ...
    emit(json.dumps(snapshot, ensure_ascii=False, indent=2) if args.json else json.dumps(snapshot, ensure_ascii=False))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ready.add_argument("--compare-only", action="store_true", help="ignore working tree in combined file checks")
